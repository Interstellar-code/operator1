...
**This is a core product decision that must be settled before Phase 1 ships.** The current schema has both `capabilities` and `routing_hints.keywords` influencing routing, with undefined conflict resolution.
...
**Open question:** Replace keyword matching with embeddings similarity for better recall on paraphrased tasks? Embeddings require an inference call at routing time; keywords are free. Decision deferred to implementation — see Open Questions below.
...
```
Agents
├── Browse        # Browse marketplace agents
├── Installed     # View installed agents
...
Dashboard page for agent marketplace:

1. **Browse tab** — Card grid of available agents with tier badges
2. **Installed tab** — Current agents with status and config
3. **Bundles tab** — Pre-packaged agent sets (SaaS Startup, Legal Practice, etc.)
4. **Settings tab** — Per-agent configuration (model, tools, permissions)
5. **Health tab** — Status overview with warnings/errors
...
### Phase 1a: Schema & Validation (spec work, no runtime)
- [ ] Finalise routing algorithm decision (keyword vs embeddings)
- [ ] Define `agent.yaml` schema with all fields (including `limits`, no `subagents`)
- [ ] Define `AGENT.md` format (prompt-only, no frontmatter)
- [ ] Create agent manifest validator with tier enforcement
- [ ] Define lock file format (`agents-lock.yaml`) per scope

### Phase 1b: Migrate Existing Agents
- [ ] Migrate all existing Matrix agents (Neo, Trinity, Morpheus, Tank, Dozer, Mouse, Oracle, Seraph, Zee, Niobe, Switch, Rex) to new `agent.yaml` + `AGENT.md` format
- [ ] Validate each migrated agent against schema
- [ ] Confirm `requires` relationships replace the old hardcoded hierarchy

### Phase 1c: CLI & Scopes
- [ ] Implement `openclaw agents` commands (install, remove, validate)
- [ ] Add installation scopes (user/project/local) with correct lock file per scope
- [ ] Implement uninstall cascade logic (`--cascade`)

### Phase 2: Registry & Runtime
- [ ] Build Agent Registry Service for gateway startup validation
- [ ] Implement degraded startup mode (disable invalid agents, gateway continues)
- [ ] Build git-based registry sync with local caching
- [ ] Implement search and browse commands
- [ ] Add health check commands (`openclaw agents health`)
- [ ] Build routing algorithm into registry (capability registry for COO)
- [ ] Add basic agent routing tests (`openclaw agents test-routing`) — needed to validate routing before Phase 3

### Phase 3: Updates & Migrations
- [ ] Implement version update flow with pre-update snapshot
- [ ] Add `openclaw agents rollback` command
- [ ] Add migration script support (`--run-migrations`)
- [ ] Add deprecation warnings and sunset flow

### Phase 4: Testing & Quality
- [ ] Build full agent testing framework (`test_routing.yaml`)
- [ ] Add `openclaw agents test` commands
- [ ] Add `openclaw agents test --all`

### Phase 5: Web UI
- [ ] Agent marketplace page in ui-next
- [ ] Agent configuration UI
- [ ] Bundle installation wizard
- [ ] Health dashboard

### Phase 6: Community
- [ ] AgentHub hosting infrastructure
- [ ] Community submission process with review/approval workflow
- [ ] Sandboxing for migration scripts (pre-requisite for community agents)
- [ ] GPG/Sigstore signing for agent packages
- [ ] Private registry support with `auth_token_env` config

---

## Open Questions (Unresolved)

### U1. Routing algorithm: keyword matching vs embeddings?

Keyword matching is free and deterministic. Embeddings give better recall on paraphrased inputs but require an inference call at routing time (latency + cost). Decision needed before Phase 1a. Recommendation: start with keywords, add embeddings as an opt-in improvement in Phase 2 once we have routing test data.

### U2. Bundle version conflicts

If Neo is installed standalone at `1.0.0` and `engineering-pack` specifies `neo@1.1.0`, what happens? Options: (a) fail with a conflict error, (b) upgrade Neo to satisfy the bundle, (c) install bundle at a version that matches existing Neo. Needs a defined conflict resolution policy before bundles ship.
